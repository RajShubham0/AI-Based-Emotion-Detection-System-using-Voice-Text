...
function detectEmotionFromText(text: string): { emotion: string; confidence: number } {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!transcript || typeof transcript !== 'string') {
      return new Response(
        JSON.stringify({ error: 'Voice transcript is required' }),
        {
          status: 400,
    ...
    const { emotion, confidence } = detectEmotionFromText(transcript);
